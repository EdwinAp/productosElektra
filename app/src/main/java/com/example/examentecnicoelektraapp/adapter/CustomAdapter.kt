...
class CustomAdapter (private val context: Activity, private val productos : List<ProductsQuery.Product>?) : ArrayAdapter<ProductsQuery.Product>(context, R.layout.productoitem, productos){
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val image = rowView.findViewById(R.id.fotoproducto) as ImageView
        ...
        if (productos?.get(position)?.lastPrice() != null) semanal.text = "$${String.format("%.2f", productos.get(position).lastPrice()!!/48)}/48 Semanales"
